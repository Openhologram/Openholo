...
int OphPointCloud::loadPointCloud(const std::string InputModelFile)
{
	...
	//File.close();
	this->InputSrcFile = InputModelFile;
	this->n_points = OphGeneration::loadPointCloudData(InputModelFile, &this->VertexArray, &this->AmplitudeArray, &this->PhaseArray);
...
{
	if (!OphGeneration::readConfigFile(InputConfigFile, this->ConfigParams))
		return false;
...
void OphPointCloud::setConfigParams(const oph::OphConfigParams & InputConfig)
{
	this->ConfigParams = InputConfig;
}

oph::OphConfigParams OphPointCloud::getConfigParams()
{
